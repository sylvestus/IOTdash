        ...
        <div class="card Recent-Users">
            <div class="card-header">
                ...
                <a href="#" class="text-dark " data-bs-toggle="modal" data-bs-target="#exampleModal"
                    data-bs-whatever="@fat"><i class="feather icon-plus-circle text-c-blue m-r-5"></i>Add Location</a>
                @endif
                @if (session('success'))
...
@endphp --}}
                        ...
                        <tbody>
                            ...
                            @foreach ($our_sites as $our_site)
                                <tr>
                                    ...
                                    <td>
                                        @if ($our_site->location)
                                            <h6 class="m-0 ">{{ $our_site->location }}</h6>
                                        @else
                                    ...
                                    @if($user_type==1)
                                    <td class="">
                                        <form action="{{ url('/delete-site-location/' . $our_site->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="bg-light p-0 m-0"
                                                style="border: none !important;"><i
                                                    class="feather icon-trash-2 color:red;"  onclick="return confirm('Are you sure you want to delete this site location ?')" >Delete</i></button>
                                        </form>
                                    </td>
                                    @endif
                                </tr>
                                ...
                                <div class="row justify-content-center">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <form class="needs-validation" novalidate
                                            action="{{ url('/add-site-location') }}" method="POST">
                                            @csrf
                                            <div class="form-group">
                                            ...
                                            <div class="form-group has-validation">
                                                <label hidden>Unique site Id</label>
                                                <input hidden type="text" class="form-control" name="site_id"
                                                    value="{{ $current_id }}" readonly placeholder="Enter user Id"
                                                    required>
                                                <div class="invalid-feedback">
                                                    Please input site id
                                                </div>
    ...
    <script>
        ...
        $(document).on('click', '[data-bs-toggle="modal"][data-bs-target="#updateModal"]', function(event) {
            ...
            var modal2 = $('#exampleModal');
            modal2.find('#userId2').val(userId);
        ...
        $(document).on('click', '[data-bs-toggle="modal"][data-bs-target="#updateModal"]', function(event) {
            ...
            var form = $('#updateForm');
            var url = '/update-site-location/' + userId;
